...
const AdminPanel = () => {
  ...
  useEffect(() => {
    ...
    if (storedAdmin) {
      const { admin_name } = JSON.parse(storedAdmin);
      setIsLoggedIn(true);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        localStorage.setItem("admin", JSON.stringify(response.data.admin)); // Store login info
      }
    } catch (err) {
  ...
  const handleLogout = () => {
    ...
    localStorage.removeItem("admin"); // Remove stored login data
  };
